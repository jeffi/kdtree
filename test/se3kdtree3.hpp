...
int volumeIndex3(const Eigen::Quaternion<_Scalar>& q) {
    int m;
...
private:
    ...
    struct Nearest {
        ...
        Nearest(const SE3KDTree3& tree, const State& key)
            : tree_(tree),
              key_(key),
              ...
              soDepth_(1),
              keyVol_(detail::volumeIndex(std::get<0>(key_))),
              explored_(0),
        ...
        template <typename _Derived>
        bool projectionOnFace(int b0, int a0, Scalar dot, const Eigen::DenseBase<_Derived>& q) {
            Eigen::Matrix<Scalar, 4, 1> p0 = q;
            p0[vol_] -= soBounds_[b0](0, a0) * dot;
            p0[(vol_ + a0 + 1)%4] -= soBounds_[b0](1, a0) * dot;
        ...
        Scalar soBoundsDistAll() {
            ...
            for (int a0 = 0 ; a0 < 3 ; ++a0) {
                ...
                if (b0 && dot[1] <= 0)
                    continue; // in bounds
                if (projectionOnFace(b0, a0, dot[b0], q))
            ...
            for (int a0 = 0 ; a0 < 3 ; ++a0) {
                ...
                for (int edge = 0 ; edge < 4 ; ++edge) {
                    ...
                    if (dotBounds(0, a2, p1) >= 0 && dotBounds(1, a2, p1) <= 0)
                        dotMax = std::max(dotMax, std::abs(p1.dot(q))); // in bounds
                }
            }
        ...
        Scalar soBoundsDistAllOld() {
            ...
            for (int a0 = 0 ; a0 < 3 ; ++a0) {
                ...
                for (int a1 = a0+1 ; (a1 = a1%3) != a0 ; ++a1) {
                    ...
                    // distance to p1 should be greater than the distance to the plane
                    assert(std::acos(std::abs(p1.normalized().dot(q))) >=
                           std::asin(std::abs(dotBounds(b0, a0, q))));
...
//         }
        ...
        void traverse(const Node* n, int depth) {
            ...
            if (rvDist > soDist) {
                ...
                if (const Node* c = n->children_[1-childNo]) {
                    ...
                    Scalar rvBoundsDistNow = rvBoundsDist();
                    if (soBoundsDistCache_ + rvBoundsDistNow <= dist_) {
                        std::swap(rvBoundsDistNow, rvBoundsDistCache_);
                        Scalar tmp = rvBounds_(rvAxis, childNo);
                        rvBounds_(rvAxis, childNo) = split;
                        traverse(c, depth+1);
                        rvBounds_(rvAxis, childNo) = tmp;
                        rvBoundsDistCache_ = rvBoundsDistNow;
                    }
                    rvDeltas_[rvAxis] = oldDelta;
                }
            } else if (soDepth_ < 3) {
                // assert(keyVol_ == detail::volumeIndex3(std::get<0>(key_)));
                soDepth_ += 2;
                assert(soDepth_ == 3);
                assert(soBoundsDistCache_ == 0);
                if (const Node *c = n->children_[keyVol_ & 1]) {
                    if (const Node *g = c->children_[(keyVol_ >> 1)]) {
                        if (std::get<0>(key_).coeffs()[vol_ = keyVol_] < 0)
                            std::get<0>(key_).coeffs() = -std::get<0>(key_).coeffs();
                        assert(soBoundsDist() == 0);
                        traverse(g, depth+2);
                    }
                    update(c);
                    if (const Node *g = c->children_[1-(keyVol_ >> 1)]) {
                        if (std::get<0>(key_).coeffs()[vol_ = keyVol_ ^ 2] < 0)
                            std::get<0>(key_).coeffs() = -std::get<0>(key_).coeffs();
                        if ((soBoundsDistCache_ = soBoundsDist()) + rvBoundsDistCache_ <= dist_)
                            traverse(g, depth+2);
                    }
                }
                update(n);
                if (const Node *c = n->children_[1-(keyVol_ & 1)]) {
                    if (const Node *g = c->children_[(keyVol_ >> 1)]) {
                        if (std::get<0>(key_).coeffs()[vol_ = keyVol_ ^ 1] < 0)
                            std::get<0>(key_).coeffs() = -std::get<0>(key_).coeffs();
                        if ((soBoundsDistCache_ = soBoundsDist()) + rvBoundsDistCache_ <= dist_)
                            traverse(g, depth+2);
                    }
                    update(c);
                    if (const Node *g = c->children_[1-(keyVol_ >> 1)]) {
                        if (std::get<0>(key_).coeffs()[vol_ = keyVol_ ^ 3] < 0)
                            std::get<0>(key_).coeffs() = -std::get<0>(key_).coeffs();
                        if ((soBoundsDistCache_ = soBoundsDist()) + rvBoundsDistCache_ <= dist_)
                            traverse(g, depth+2);
                    }
        ...
        void traverseRoots(const Node* root) {
            traverse(root, 1);
        }
    };
    
public:
    SE3KDTree3(_TtoKey tToKey, const Eigen::Array<_Scalar, 3, 2>& bounds)
        : tToKey_(tToKey),
          bounds_(bounds),
          size_(0),
          depth_(0),
    ...
    void add(const _T& t) {
        ...
        for ( ;; p = c, ++depth) {
            ...
            if (rvDist > soDist) {
                ...
                if ((c = p->children_[childNo = (vol >> (soDepth-1)) & 1]) == nullptr)
                    break;
                ++soDepth;
            } else {
